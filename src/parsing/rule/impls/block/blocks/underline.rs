...
fn parse_fn<'r, 't>(
    ...
    let body = parser.get_body_elements(&BLOCK_UNDERLINE, false)?;
...
mod tests {
    ...
    #[test]
    fn underline_block_alias_wraps_inline_body() {
        let page_info = PageInfo::dummy();
        let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);
        let tokenization = crate::tokenize("[[u]]underlined text[[/u]]");
        ...
        let [Element::Container(container)] = paragraph.elements() else {
            panic!(
                "expected one underline container, got {:?}",
                paragraph.elements()
            );
        };

        assert_eq!(container.ctype(), ContainerType::Underline);
        assert!(container.attributes().get().is_empty());
        assert_eq!(
            container.elements(),
            &[
                Element::Text(cow!("underlined")),
                Element::Text(cow!(" ")),
