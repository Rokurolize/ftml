...
The current stable inventory has 227 cases: 199 PagePreview-compatible cases, 27 caller-runtime cases, and 1 not-applicable case. The runtime and not-applicable rows are accounted for with explicit reasons and are not sent to the anonymous PagePreview capture lane.
...
Split the preview-compatible inventory into fresh JSONL batches of at most 16 cases and verify the current 199-row selection:
...
test "$(wc -l "$PARITY_TMP"/batches/cases-*.jsonl | tail -n 1 | awk '{print $1}')" -eq 199
```
