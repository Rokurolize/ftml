...
Reference batches `references-20260817-05.jsonl` and `references-20260817-06.jsonl` freeze the focused `getAttrs`-versus-generic-attribute typography family. The `div` legacy-`getAttrs` control matches and has a reviewed snapshot. Wikidot preserves authored ASCII inside generic `span` attribute values, while `Layout::Wikidot` applies prose typography there. The number-space witness (`style="padding: 0 2px;"`), the unit-space witness (`data-x="41 px"`), the ellipsis witness (`data-x="x....x"`), and the angle-quote witness (`data-x="<<A>>"`) all differ only in the DOM tree while DOM signature and visible text match. The number-space and unit-space rows are unresolved FTML-owned mismatches under issue #581; the ellipsis and angle-quote rows are unresolved under issue #582. None of the mismatch rows has a passing snapshot.
