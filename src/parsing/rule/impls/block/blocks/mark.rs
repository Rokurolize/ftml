...
fn parse_fn<'r, 't>(
    ...
    let body = parser.get_body_elements(&BLOCK_MARK, false)?;
...
mod tests {
    ...
    #[test]
    fn mark_block_alias_wraps_inline_body() {
        let page_info = PageInfo::dummy();
        let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);
        let tokenization = crate::tokenize("[[highlight]]marked text[[/highlight]]");
        ...
        let [Element::Container(container)] = paragraph.elements() else {
            panic!(
                "expected one mark container, got {:?}",
                paragraph.elements()
            );
        };

        assert_eq!(container.ctype(), ContainerType::Mark);
        assert!(container.attributes().get().is_empty());
        assert_eq!(
            container.elements(),
            &[
                Element::Text(cow!("marked")),
                Element::Text(cow!(" ")),
