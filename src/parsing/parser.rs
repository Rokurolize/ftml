...
// the caller's stack for nesting, so keep the headroom deliberately modest:
// the library must return a bounded parse error on ordinary runtime threads
// rather than relying on a larger test-only stack.
pub(crate) const DEFAULT_MAX_RECURSION_DEPTH: usize = 100;
pub(crate) const DEEP_MAX_RECURSION_DEPTH: usize = 128;
