...
fn parse_fn<'r, 't>(
    ...
    let body = parser.get_body_elements(&BLOCK_SUPERSCRIPT, false)?;
...
mod tests {
    ...
    #[test]
    fn superscript_block_alias_wraps_inline_body() {
        let page_info = PageInfo::dummy();
        let settings = WikitextSettings::from_mode(WikitextMode::Page, Layout::Wikidot);
        let tokenization = crate::tokenize("[[super]]super text[[/super]]");
        ...
        let [Element::Container(container)] = paragraph.elements() else {
            panic!(
                "expected one superscript container, got {:?}",
                paragraph.elements()
            );
        };

        assert_eq!(container.ctype(), ContainerType::Superscript);
        assert!(container.attributes().get().is_empty());
        assert_eq!(
            container.elements(),
            &[
                Element::Text(cow!("super")),
                Element::Text(cow!(" ")),
