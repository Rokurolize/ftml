...
printf 'FTML Codex Cloud maintenance revision %s\n' "$script_revision"
...
prepare_process_environment() {
  ...
  if [[ ! -d "$state_dir" || -L "$state_dir" ]]; then
    printf 'Trusted FTML Codex state is missing; reset the environment cache.\n' >&2
    return 1
  fi
  if [[ ! -f "$trusted_requirements" || -L "$trusted_requirements" ]]; then
    printf 'Trusted Python requirements are missing; reset the environment cache.\n' >&2
    return 1
  fi
  for path in "$cargo_home" "$tool_root"; do
    if [[ ! -d "$path" || -L "$path" ]]; then
      printf 'Trusted FTML Codex tool state is missing; reset the environment cache: %s\n' "$path" >&2
      return 1
    fi
  done
  ...
  for config in /.cargo/config /.cargo/config.toml "$cargo_home/config" "$cargo_home/config.toml"; do
    if [[ -e "$config" || -L "$config" ]]; then
      printf 'Unexpected Cargo configuration could execute helpers during maintenance: %s\n' "$config" >&2
      return 1
    fi
  done
...
PY
  ...
  if ! cmp -s -- "$repo/scripts/check_conf-requirements.txt" "$trusted_requirements"; then
    printf 'Python requirements changed; update the reviewed environment scripts and reset the cache.\n' >&2
    return 1
  fi
...
for command_name in git python3 rustup; do
  if ! command -v "$command_name" >/dev/null 2>&1; then
    printf '%s is missing; reset the Codex environment cache.\n' "$command_name" >&2
    exit 1
  fi
done
...
# Fetch from / so task-local Cargo configuration and credential providers are outside Cargo's discovery path.
fetch_repository_dependencies

printf 'FTML Codex Cloud maintenance completed.\n'
